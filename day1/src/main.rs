fn main() {
    let source_data = std::fs::read_to_string("./day1/data011.txt").unwrap();

    let mut sum_values = 0;
    let mut list_sums: Vec<i32> = Vec::new();

    for string in source_data.lines() {
        if !string.is_empty() {
            let value: i32 = string.parse().unwrap();
            sum_values += value;
        } else {
            // next gnome
            list_sums.push(sum_values);
            sum_values = 0;
        }
    }

    let max_sum = list_sums.iter().max();
    match max_sum {
    ...
    // list_sums.sort();
    // list_sums.reverse();
    list_sums.sort_by(|a, b| b.cmp(a));
    list_sums.truncate(3);

    let joined: String = list_sums.iter().map(|&id| id.to_string() + ", ").collect();
    println!("List: {joined}");
    let sum: i32 = list_sums.iter().sum();
    println!("Sum: {sum}");
    // END DAY 1 PART 2
}
