...
fn main() {
    let source_strings = std::fs::read_to_string("./day2/day021.txt").unwrap();

    let rps_translation = HashMap::from([("A", "X"), ("B", "Y"), ("C", "Z")]);
    let score_table = HashMap::from([("X", 1), ("Y", 2), ("Z", 3)]);

    let mut total_score = 0;
    for round in source_strings.lines() {
        if let Some((elf, me)) = round.split_once(" ") {
            let elf = rps_translation[elf];
            let element_score = score_table[me];
    ...
    println!("Part1: I've scored: {total_score}");

    let mut total_score = 0;

    let shapes = Vec::from(["X", "Y", "Z"]);

    for round in source_strings.lines() {
        if let Some((elf, result)) = round.split_once(" ") {
            let elf = rps_translation[elf];
            let shape_idx = shapes.iter().position(|&v| v == elf).unwrap();

            // LOSE
            if result == "X" {
                let me_shape = shapes[(shape_idx + 2) % 3];
                let score = score_table[me_shape];
                total_score += score;
            }
            // DRAW > Pick the same shape as the elf
            if result == "Y" {
                total_score += 3;
                let me_shape = shapes[shape_idx];
                let score = score_table[me_shape];
                total_score += score;
            }
            // WIN > Pick the winning shape
            if result == "Z" {
                total_score += 6;
                let me_shape = shapes[(shape_idx + 1) % 3];
                let score = score_table[me_shape];
                total_score += score;
            }
        } else {
    ...
    println!("Part2: I've scored: {total_score}");
}
// ---
